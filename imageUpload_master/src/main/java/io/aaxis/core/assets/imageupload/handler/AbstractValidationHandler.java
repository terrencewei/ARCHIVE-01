...
public abstract class AbstractValidationHandler<T, U extends Validator> {
    ...
    public final Mono<ServerResponse> handleRequestMultipartData(final ServerRequest pRequest,
        ...
        return pRequest
                .multipartData()
                .flatMap(pMultiValueMap -> multipartDataToMono(pMultiValueMap))
                .flatMap(requestWrapper -> doValidation(pRequest, pAdditionalData, requestWrapper,
                        new DirectFieldBindingResult(requestWrapper, this.validationClass.getName())));
    }
     ...
     *
     * @param requestWrapperObject     the validation object
     * @param pRequest        the request
     * @param pAdditionalData the additional data holder when process validation event
     * @return server response
     */
    protected abstract Mono<ServerResponse> onValidationPassed(T requestWrapperObject, final ServerRequest pRequest,
            Map<String, Object> pAdditionalData);
    ...
    private Mono<? extends ServerResponse> doValidation(ServerRequest pRequest, Map<String, Object> pAdditionalData,
            T requestWrapper, Errors pErrors) {
        this.validator.validate(requestWrapper, pErrors);
        return ErrorUtils.hasErrors(pErrors) ?
                onValidationPassed(requestWrapper, pRequest, pAdditionalData) :
                onValidationErrors(pErrors, requestWrapper, pRequest, pAdditionalData);
    }
